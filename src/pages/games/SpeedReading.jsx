...
export default function SpeedReading() {
    ...
    if (authUser) {
      ...
      try {
        await supabase.from('test_attempts').insert({
          user_id: uid, exam_name: 'game_speedreading',
          subject: 'English RC', score, total: questions.length * 10,
  ...
  const shareResult = () => {
    const text = `📖 Speed Reading: ${score} pts | Combo x${maxCombo} on TryIT! tryiteducations.net`
    if (navigator.share) navigator.share({ title: `Speed Reading Score`, text })
  ...
  if (phase === 'intro') return (
    <div style={{ minHeight:'100vh', background:bg, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:24, fontFamily:'Inter,sans-serif' }}>
      ...
      <div style={{ textAlign:'center', maxWidth:380, width:'100%' }}>
        ...
        <p style={{ color:'#fff',fontFamily:'Poppins,sans-serif',fontWeight:900,fontSize:28,margin:'0 0 8px' }}>Speed Reading</p>
        <p style={{ color:'rgba(255,255,255,0.5)',fontSize:13,margin:'0 0 6px' }}>Reading comprehension under timer</p>
        <p style={{ color:C1,fontSize:11,fontWeight:700,margin:'0 0 24px',background:`${C1}18`,padding:'6px 14px',borderRadius:20,display:'inline-block' }}>⚡ RC = 20 to 30 percent of English section!</p>
